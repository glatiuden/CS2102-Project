...
import { CareTaker, deleteCareTaker, getAllFullTimers } from '../../database/CareTakerManager';
import { getFTSalary } from '../../database/DashboardManager';
...
const PageFullTimerManagement = () => {
    ...
    const [openFullTimerDialog, setOpenFullTimerDialog] = useState(false);
    ...
    const init = () => {
        getAllFullTimers().then((fullTimers) => {
            if (fullTimers) {
                setFullTimerList(fullTimers.map(val => ({
                    email: val.email,
                    name: val.name,
                    address: val.address,
                    region: val.region,
    ...
    const onDeleteSubmit = (fullTimerInfo: CareTaker) => {
        setFullTimerList([]);
        deleteCareTaker(fullTimerInfo).then((result) => {
            if (result === 1) {
                notifySuccess("You have successfully deleted " + "'" + fullTimerInfo.name + "'" + "!");
                closeDialog();
            } else {
    ...
    const onFullTimerClick = (fullTimerInfo: CareTaker) => {
        setOpenFullTimerDialog(true);
        setSelectedFullTimerInfo(fullTimerInfo);
        getSalaryReport(fullTimerInfo.email, toDate.getMonth() + 1).then((report) => {
            if (report != null) {
                setSalaryReport(report);
                getSalaryReport(fullTimerInfo.email, toDate.getMonth()).then((lastReport) => {
                    if (lastReport != null) {
        ...
        getAverageRatings(fullTimerInfo.email).then((ratings) => {
            if (ratings != null) {
                setRatings(ratings);
            }
        })
    ...
    const columns = [
        ...
        },
        {
            ...
            sortable: true,
            cell: (row: any) =>
                <Link onClick={() => onFullTimerClick(row)} underline="always">
    ...
    return !loading ? (
        <div style={{ width: '100%' }}>
            ...
            <Box width="100%">
                <Dialog open={openFullTimerDialog} aria-labelledby="form-dialog-title" maxWidth="md" fullWidth={true}>
                    <DialogTitle id="form-dialog-title">{selectedFullTimerInfo ? selectedFullTimerInfo.name : ''}</DialogTitle>
                    ...
                    <DialogActions>
                        <Button variant="contained" color="primary" onClick={() => setOpenFullTimerDialog(false)}>
                            Close
                        </Button>
                    </DialogActions>
                </Dialog>
